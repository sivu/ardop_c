...
UCHAR bytFailedData[3120];		// do we rally need that much ????
...
sendit:
		...
		{
			EncLen = EncodeOFDMData(bytFrameType, bytDataToSend, Len, bytEncodedBytes);
			RemoveDataFromQueue(Len);		// No ACKS in FEC
			NextOFDMBlock = 0;				// Reset for next frame
